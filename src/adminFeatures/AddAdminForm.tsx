      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex justify-end gap-3 mt-2">
          <button
            ...
            disabled={isPending}
            onClick={() => window.history.back()}
            className="px-5 py-2 text-xs font-bold text-slate-700 border border-slate-300 bg-white hover:bg-slate-50 active:bg-slate-100 rounded-xl transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={isPending}
            className="inline-flex items-center gap-1.5 px-5 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 rounded-xl transition-colors shadow-sm shadow-indigo-100 disabled:opacity-50"
          >
            {isPending ? (
              <div className="w-3.5 h-3.5 border-2 border-white border-b-transparent rounded-full animate-spin" />
            ) : (
