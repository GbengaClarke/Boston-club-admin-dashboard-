...
export function SeedOperations() {
  ...
  const toastConfig = {
    duration: 8000, // Longer visibility context for critical warnings
    position: "top-center" as const,
    style: {
      minWidth: "360px",
      padding: "16px",
      background: "#1e293b",
      color: "#fff",
      border: "1px solid #451a03", // Subtle warning accent border
    },
  };
      ...
      (t) => (
        <div className="flex flex-col gap-3">
          <div className="space-y-1">
            <p className="text-sm font-bold text-rose-400 flex items-center gap-1.5 uppercase tracking-wider">
              ⚠️ Critical Warning
            </p>
            <p className="text-xs text-slate-300 leading-relaxed">
              This will permanently delete all data records including products,
              order items, history metrics, and consumer reviews. This action is
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  toast.success("Database wiped entirely clean!");
                } catch (error: any) {
                  toast.error(`Purge execution breakdown: ${error.message}`);
                } finally {
                  setIsWiping(false);
                }
              }}
              className="px-3 py-1.5 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-md transition-colors shadow-sm"
            >
              Wipe Tables
            </button>
          </div>
  ...
  return (
    <div className="px-4 pb-4 flex flex-col gap-1 border-b border-slate-800/60">
      ...
      {/* Operation 1: Populating/Seeding */}
      <button
        onClick={handleStoreSeed}
      ...
      {/* Operation 2: Destructive Clear Layer */}
      <button
        onClick={handleDatabaseWipe}
        disabled={isSeeding || isWiping}
        className="flex items-center gap-3 px-3 py-2 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-slate-800/80 transition-colors text-left disabled:opacity-50"
      >
        {isWiping ? (
        ...
        <span className="truncate">
          {isWiping ? "Purging Tables..." : "Wipe Entire Database"}
        </span>
      </button>
    </div>
  );
